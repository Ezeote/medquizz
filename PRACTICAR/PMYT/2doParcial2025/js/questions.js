let questions = [

{
    "numb": 1,
    "question": "Indique cuál es la estrategia más apropiada para minimizar la supresión del eje hipotálamohipófiso-suprarrenal en terapia prolongada con prednisona: ",
    "answer": [
      "d) Administración con reducción gradual de la dosis. "
    ],
    "options": [
      "a) Uso exclusivo de formulaciones tópicas. ",
      "b) Administración continua a dosis nocturnas. ",
      "c) Suspensión inmediata cuando mejore la clínica. ",
      "d) Administración con reducción gradual de la dosis. "
    ],
    "discipline": "Farmaco",
    "comment": "La administración prolongada de corticoides sistémicos como la prednisona causa supresión del eje hipotálamo-hipófiso-suprarrenal (HHS) mediante retroalimentación negativa sobre la CRH y la ACTH. Para evitar una insuficiencia suprarrenal aguda (crisis addisoniana) secundaria a la atrofia suprarrenal, la retirada del fármaco debe realizarse mediante un descenso paulatino de la dosis (tapering), permitiendo la recuperación fisiológica progresiva del eje."
  },
  {
    "numb": 2,
    "question": "Señale cuál es un riesgo clínicamente relevante de los anticuerpos monoclonales anti-TNF (factor de necrosis tumoral), como infliximab: ",
    "answer": [
      "c) Reactivación de tuberculosis latente. "
    ],
    "options": [
      "a) Hipercalcemia aguda. ",
      "b) Hipotiroidismo irreversible. ",
      "c) Reactivación de tuberculosis latente. ",
      "d) Hemólisis por déficit de glucosa-6-fosfato deshidrogenasa. "
    ],
    "discipline": "Farmaco",
    "comment": "El factor de necrosis tumoral alfa (TNF-α) es una citocina crítica para la formación y mantenimiento estructural de los granulomas que contienen a Mycobacterium tuberculosis. La inhibición de TNF-α con monoclonales como infliximab desestructura el granuloma, permitiendo la diseminación bacilar y la reactivación de una tuberculosis latente, por lo que es obligatorio realizar cribado prevencionista (PPD e IGRA) antes de iniciar la terapia."
  },
  {
    "numb": 3,
    "question": "Indique qué conducta inicial tomaría ante depresión respiratoria por sobredosis de opioides: ",
    "answer": [
      "b) Naloxona intravenosa. "
    ],
    "options": [
      "a) Carbón activado oral. ",
      "b) Naloxona intravenosa. ",
      "c) Furosemida intravenosa. ",
      "d) Flumazenilo intravenoso. "
    ],
    "discipline": "Farmaco",
    "comment": "La naloxona es un antagonista puro de los receptores opioides (con alta afinidad por los receptores mu-µ). Antagoniza de manera competitiva la acción de los opioides en el sistema nervioso central, revirtiendo rápidamente la depresión respiratoria, la miosis y la sedación potencialmente mortales en casos de sobredosis."
  },
  {
    "numb": 4,
    "question": "Considerando los criterios de selección de medicamentos en infección urinaria no complicada en mujer joven sin comorbilidades, ¿Cuál considera una elección empírica racional? ",
    "answer": [
      "d) Nitrofurantoína. "
    ],
    "options": [
      "a) Penicilina. ",
      "b) Cefradina. ",
      "c) Clindamicina. ",
      "d) Nitrofurantoína. "
    ],
    "discipline": "Farmaco",
    "comment": "La nitrofurantoína es una de las opciones de primera línea para la cistitis aguda no complicada en mujeres jóvenes. Alcanza elevadas concentraciones en la luz urinaria con mínima absorción tisular sistémica, bajo impacto negativo en la microbiota gastrointestinal y un perfil sostenido de muy baja resistencia por parte de Escherichia coli."
  },
  {
    "numb": 5,
    "question": "Considerando los criterios de selección de medicamentos en neumonía adquirida en la comunidad sin elementos de gravedad en adulto joven previamente sano, ¿Cuál considera una elección empírica racional? ",
    "answer": [
      "a) Amoxicilina. "
    ],
    "options": [
      "a) Amoxicilina. ",
      "b) Ceftriaxona. ",
      "c) Levofloxacino. ",
      "d) Trimetoprim sulfametoxazol. "
    ],
    "discipline": "Farmaco",
    "comment": "En neumonía adquirida en la comunidad (NAC) de manejo ambulatorio en un adulto joven previamente sano, el etiológico bacteriano predominante es Streptococcus pneumoniae. La amoxicilina oral a dosis altas es el tratamiento de primera línea por su elevada eficacia clínica, perfil de seguridad y menor espectro que evita la selección innecesaria de resistencias en comparación con fluoroquinolonas o cefalosporinas de amplio espectro."
  },
  {
    "numb": 6,
    "question": "Marque cuál es un efecto adverso grave de macrólidos:  ",
    "answer": [
      "d) Prolongación del intervalo QT. "
    ],
    "options": [
      "a) Litiasis renal. ",
      "b) Hipokalemia severa.  ",
      "c) Hemólisis intravascular. ",
      "d) Prolongación del intervalo QT. "
    ],
    "discipline": "Farmaco",
    "comment": "Los macrólidos (azitromicina, claritromicina, eritromicina) bloquean los canales de potasio IKr (HERG) en las células miocárdicas, lo que retrasa la repolarización ventricular y se manifiesta en el electrocardiograma como una prolongación del intervalo QT, incrementando el riesgo de arritmias ventriculares graves como la Torsades de Pointes."
  },
  {
    "numb": 7,
    "question": "especto a fármacos antirretrovirales, ¿qué principio de uso racional es prioritario? ",
    "answer": [
      "c) Combinaciones de dos o tres fármacos activos para prevenir resistencias. "
    ],
    "options": [
      "a) Suspender el tratamiento ante la viremia baja. ",
      "b) Evitar pruebas de resistencia por su elevado costo. ",
      "c) Combinaciones de dos o tres fármacos activos para prevenir resistencias. ",
      "d) Monoterapia con inhibidor de proteasa potenciado para evitar interacciones. "
    ],
    "discipline": "Farmaco",
    "comment": "Dado el elevado ritmo de replicación del VIH y la alta tasa de mutación intrínseca de la transcriptasa inversa, la monoterapia genera rápidamente cepas resistentes. El pilar fundamental de la terapia antirretroviral (TARV) es la combinación de múltiples fármacos activos de diferentes familias terapéuticas para lograr una supresión virológica duradera e impedir la selección de mutaciones de resistencia."
  },
  {
    "numb": 8,
    "question": "¿Cuál de los siguientes efectos adversos está descrito para metoclopramida? ",
    "answer": [
      "d) Síntomas extrapiramidales. "
    ],
    "options": [
      "a) Hipercalcemia. ",
      "b) Crisis hipertiroidea. ",
      "c) Depresión respiratoria. ",
      "d) Síntomas extrapiramidales. "
    ],
    "discipline": "Farmaco",
    "comment": "La metoclopramida es un antiemético y procinético que actúa bloqueando los receptores dopaminérgicos D2 central y periféricamente. Al bloquear la neurotransmisión dopaminérgica en la vía nigroestriada de los ganglios basales, puede desencadenar efectos adversos de tipo extrapiramidal como distonías agudas, acatisia, parkinsonismo secundario y discinesia tardía."
  },
  {
    "numb": 9,
    "question": "Marque cuál de los siguientes antidepresivos está aprobado para dolor neuropático: ",
    "answer": [
      "c) Duloxetina. "
    ],
    "options": [
      "a) Bupropión. ",
      "b) Fluoxetina. ",
      "c) Duloxetina. ",
      "d) Mirtazapina. "
    ],
    "discipline": "Farmaco",
    "comment": "La duloxetina es un inhibidor de la recaptación de serotonina y noradrenalina (IRSN). La potenciación de la vía noradrenérgica y serotoninérgica en las vías descendentes inhibitorias del dolor a nivel del astas posteriores de la médula espinal le confiere una clara eficacia analgésica, contando con aprobación oficial para patologías como la neuropatía diabética periférica y la fibromialgia."
  },
  {
    "numb": 10,
    "question": "¿Cuál de los siguientes efectos adversos se asocia con el uso de inhibidores selectivos de recaptación de serotonina? ",
    "answer": [
      "b) Hiponatremia. "
    ],
    "options": [
      "a) Hiperkalemia. ",
      "b) Hiponatremia. ",
      "c) Hipoglucemia persistente. ",
      "d) Acortamiento del intervalo QT "
    ],
    "discipline": "Farmaco",
    "comment": "Los inhibidores selectivos de la recaptación de serotonina (ISRS) pueden inducir el Síndrome de Secreción Inadecuada de Hormona Antidiurética (SIADH), estimulando la liberación central de vasopresina. Esto produce reabsorción renal excesiva de agua e hiponatremia dilucional, complicación observada con mayor frecuencia en adultos mayores."
  },
  {
    "numb": 11,
    "question": "Marque cuál antiepiléptico es teratogénico con riesgo de defectos del tubo neural: ",
    "answer": [
      "d) cÁcido valproico. "
    ],
    "options": [
      "a) Lamotrigina. ",
      "b) Gabapentina. ",
      "c) Levetiracetam. ",
      "d) cÁcido valproico. "
    ],
    "discipline": "Farmaco",
    "comment": "El ácido valproico exhibe un elevado potencial teratogénico por alteración del metabolismo del folato y regulación epigenética. Su exposición intrauterina durante el primer trimestre incrementa de 10 a 20 veces el riesgo de defectos del cierre del tubo neural (como espina bífida), además de malformaciones craneofaciales, cardíacas y déficit cognitivo posterior."
  },
  {
    "numb": 12,
    "question": "Indique cuál es un efecto adverso grave de metimazol: ",
    "answer": [
      "c) Agranulocitosis. "
    ],
    "options": [
      "a) Hiperglucemia. ",
      "b) Hipercalcemia. ",
      "c) Agranulocitosis. ",
      "d) Nefritis intersticial crónica universal. "
    ],
    "discipline": "Farmaco",
    "comment": "La agranulocitosis (recuento absoluto de neutrófilos < 500/mm³) es la complicación idiosincrásica más grave y potencialmente letal vinculada a las antitiroideos tipo tionamida (metimazol, propiltiouracilo). Requiere la advertencia al paciente de suspender inmediatamente el fármaco y consultar si presenta fiebre o sintomatología infecciosa faríngea."
  },
  {
    "numb": 13,
    "question": "¿Qué resultado serológico esperaría encontrar en una paciente con lupus eritematoso sistémico clínicamente inactivo? ",
    "answer": [
      "d) Valores normales de proteína C reactiva (PCR) "
    ],
    "options": [
      "a) Disminución de las proteínas del complemento C3 y C4 ",
      "b) (Niveles elevados de anticuerpos anti-ADN doble cadena (anti-dsDNA) ",
      "c) Aumento de la velocidad de eritrosedimentación (VES) ",
      "d) Valores normales de proteína C reactiva (PCR) "
    ],
    "discipline": "Patologia",
    "comment": "El Lupus Eritematoso Sistémico (LES) se caracteriza por una disociación clásica: elevación de la VES en los brotes pero niveles normales o muy levemente elevados de PCR (debido a la inhibición hepática de PCR mediada por interferón alfa). Cuando la enfermedad está inactivada, tanto los reactantes de fase aguda como la PCR se mantienen dentro de límites normales, y el complemento (C3 y C4) no consumido retorna a sus cifras fisiológicas."
  },
  {
    "numb": 14,
    "question": "¿Cuál es el principal autoantígeno blanco de las respuestas inmunitarias en la enfermedad de Graves-Basedow? ",
    "answer": [
      "b) Receptor de TSH "
    ],
    "options": [
      "a) Tiroglobulina ",
      "b) Receptor de TSH ",
      "c) Peroxidasa tiroidea ",
      "d) Tiroxina (T4) "
    ],
    "discipline": "Patologia",
    "comment": "La enfermedad de Graves-Basedow es una patología autoinmune organoespecífica caracterizada por la presencia de autoanticuerpos estimulantes dirigidos contra el receptor de hormona estimulante de la tiroides (TSH-R). La unión de estos anticuerpos (TRAb) estimula constitutivamente la vía del AMPc en las células foliculares tiroideas, provocando hiperfunción e hiperplasia glandular."
  },
  {
    "numb": 15,
    "question": "Una ferritina elevada con sideremia baja y transferrina normal es típica de la anemia de etiología:  ",
    "answer": [
      "d) Inflamatoria "
    ],
    "options": [
      "a) Ferropénica ",
      "b) Hemolítica con hemólisis intravascular ",
      "c) Megaloblástica por déficit de vitamina B12 ",
      "d) Inflamatoria "
    ],
    "discipline": "Patologia",
    "comment": "La anemia por enfermedad crónica o inflamatoria está mediada por un aumento de hepcidina inducido por citocinas proinflamatorias (principalmente IL-6). La hepcidina degrada la ferroportina, bloqueando la salida de hierro desde los macrófagos. Esto genera secuestro reticuloendotelial de hierro con ferritina alta (por ser además reactante de fase aguda) e hiposideremia con saturación de transferrina baja a normal."
  },
  {
    "numb": 16,
    "question": "La prueba de Coombs directa se utiliza para detectar: ",
    "answer": [
      "b) Anticuerpos fijados a eritrocitos "],
    "options": [
      "a) Inmunocomplejos Circulantes en plasma ",
      "b) Anticuerpos fijados a eritrocitos ",
      "c) Anticuerpos circulantes en plasma ",
      "d) Anticuerpos fijados a plaquetas "
    ],
    "discipline": "Patologia",
    "comment": "La prueba de Coombs directa o Direct Antiglobulin Test (DAT) detecta la presencia in vivo de inmunoglobulinas (IgG) o factores del complemento (C3d) adheridos directamente sobre la membrana de los eritrocitos del paciente. Es el pilar fundamental para el diagnóstico de la anemia hemolítica autoinmune y la enfermedad hemolítica del recién nacido."
  },
  {
    "numb": 17,
    "question": "¿Qué resultado de laboratorio permite diferenciar la anemia por déficit de vitamina B12 de aquella por déficit de ácido fólico? ",
    "answer": [
      "c) Elevación del ácido metilmalónico "
    ],
    "options": [
      "a) Aumento de la homocisteína ",
      "b) Presencia de neutrófilos hipersegmentados en sangre periférica ",
      "c) Elevación del ácido metilmalónico ",
      "d) Incremento de la bilirrubina indirecta "
    ],
    "discipline": "Patologia",
    "comment": "La vitamina B12 actúa como cofactor indispensable en la conversión enzimática de metilmalonil-CoA a succinil-CoA; por ende, su deficiencia provoca la acumulación característica de ácido metilmalónico en plasma y orina. En la deficiencia de folato (vitamina B9) los niveles de ácido metilmalónico son normales (mientras que la homocisteína se eleva en ambos déficits)."
  },
  {
    "numb": 18,
    "question": "¿Qué trastorno hemorrágico cursa con sangrados típicos del compromiso de la hemostasia primaria, pero puede acompañarse de alteraciones en pruebas de coagulación? ",
    "answer": [
      "c) Enfermedad de von Willebrand "
    ],
    "options": [
      "a) Hemofilia B ",
      "b) Trombastenia de Glanzmann ",
      "c) Enfermedad de von Willebrand ",
      "d) Trombocitopatía inducida por ácido acetilsalicílico "
    ],
    "discipline": "Patologia",
    "comment": "La enfermedad de von Willebrand altera la hemostasia primaria al dificultar la adhesión e interacción plaquetaria con el subendotelio (manifestado por hematomas mucocutáneos y epistaxis). Asimismo, el Factor von Willebrand actúa como proteína transportadora y estabilizadora del Factor VIII de la coagulación; al descender el FvW, disminuye el Factor VIII plasmático, prolongando el aPTT (vía intrínseca)."
  },
  {
    "numb": 19,
    "question": "Durante el estudio de un paciente con prolongación del tiempo de tromboplastina parcial activada (aPTT), se realiza una prueba de mezclas. El aPTT se corrige completamente. ¿Cuál es la interpretación más probable? ",
    "answer": [
      "b) Déficit de factor VIII "
    ],
    "options": [
      "a) Presencia de un inhibidor adquirido de un factor de la vía intrínseca ",
      "b) Déficit de factor VIII ",
      "c) Presencia de anticoagulante lúpico ",
      "d) Déficit de factor VII ",
    ],
    "discipline": "Patologia",
    "comment": "La prueba de mezclas (plasma del paciente mezclado 1:1 con plasma normal) distingue entre déficit de factores e inhibidores. Si el aPTT corrige tras la mezcla, indica que el plasma normal aportó el factor faltante (>50% de actividad), confirmando una deficiencia cuantitativa de un factor de la vía intrínseca (como el Factor VIII en Hemofilia A). Si no corrigiera, sugeriría la presencia de un inhibidor circulante (ej. anticoagulante lúpico o anticuerpos neutralizantes)."
  },
  {
    "numb": 20,
    "question": "¿Cuál es el principal blanco antigénico de los anticuerpos antifosfolipídicos clínicamente relevantes? ",
    "answer": [
      "a) B2-glicoproteína I "
    ],
    "options": [
      "a) B2-glicoproteína I ",
      "b) Anexina V ",
      "c) Fosfatidilserina ",
      "d) Protrombina "
    ],
    "discipline": "Patologia",
    "comment": "En el Síndrome Antifosfolipídico (SAF), los anticuerpos patógenos no están dirigidos contra los fosfolípidos puros, sino contra proteínas plasmáticas fijadas a fosfolípidos aniónicos de membrana. La Beta-2-glicoproteína I (β2-GPI) constituye el autoantígeno blanco más relevante para la inducción del estado procoagulante e interferencia endotelial."
  },
  {
    "numb": 21,
    "question": "El remodelado óseo es importante en la determinación de la calidad ósea y su alteración puede llevar a osteoporosis, en cuanto a las vías que lo regulan, señale lo correcto: ",
    "answer": [
      "a) La activación de la vía RANK-RANK ligando estimula la reabsorción ósea. "
    ],
    "options": [
      "a) La activación de la vía RANK-RANK ligando estimula la reabsorción ósea. ",
      "b) La esclerostina es un importante estímulo para la formación ósea. ",
      "c) EI TNF alfa estimulan la formación ósea. ",
      "d) Los estrógenos estimulan la reabsorción ósea e inhiben la osteoformación "
    ],
    "discipline": "Patologia",
    "comment": "La interacción de RANKL (producido por osteoblastos y osteocitos) con el receptor RANK ubicado en los precursores osteoclásticos es la vía primordial para inducir la diferenciación, maduración, actividad resorbiológica y supervivencia de los osteoclastos, promoviendo la resorción del tejido óseo."
  },
  {
    "numb": 22,
    "question": "Cuál es el principal mecanismo mediante el cual los cristales de urato monosódico inducen la respuesta inflamatoria en la gota? ",
    "answer": [
      "a) Son fagocitados, rompen el fagolisosoma y activan el inflamosoma, lo que induce la liberación de IL-1. "
    ],
    "options": [
      "a) Son fagocitados, rompen el fagolisosoma y activan el inflamosoma, lo que induce la liberación de IL-1. ",
      "b) Permanecen extracelulares y estimulan directamente la liberación de IL-1 sin necesidad de fagocitosis ",
      "c) Inhiben la formación del inflamosoma, aumentando así la producción de citocinas proinflamatorias. ",
      "d) Se disuelven rápidamente en el citoplasma, evitando la activación inmunitaria "
    ],
    "discipline": "Patologia",
    "comment": "Los cristales de urato monosódico en la cavidad articular son fagocitados por macrófagos y monocitos. Su acumulación intralisosomal altera la membrana fagolisosómica, liberando enzimas al citosol que activan el complejo del inflamosoma NLRP3. Este activa la caspasa-1, la cual escinde la pro-IL-1β produciendo abundante Interleuquina-1 beta activa, desencadenante central de la crisis inflamatoria aguda de gota."
  },
  {
    "numb": 23,
    "question": "Con respecto a las complicaciones agudas por consumo de MDMA (metilendioximetanfetamina), señale lo correcto:  ",
    "answer": [
      "b) El consumo de alcohol asociado puede incrementar el riesgo de rabdomiólisis e injuria renal aguda "
    ],
    "options": [
      "a) Se manifiesta fundamentalmente por un efecto disociativo con depresión de conciencia  y confusión dado su efecto antagonista sobre el receptor NMDA b) ",
      "b) El consumo de alcohol asociado puede incrementar el riesgo de rabdomiólisis e injuria renal aguda ",
      "c) La hipertermia es debida a una disfunción del metabolismo del calcio a nivel muscular (hipertermia maligna) ocasionada por el MDMA ",
      "d) La hiponatremia está relacionada a la ingesta abundante de agua para evitar la deshidratación y no por efecto tóxico directo del MDMA "
    ],
    "discipline": "Patologia",
    "comment": "La coingesta de alcohol y MDMA aumenta la deshidratación, la desacoplación térmica corporal y la sobreactividad muscular involuntaria. Esto potencia fuertemente el riesgo de sufrir síndrome de hipertermia, rabdomiólisis masiva con liberación de mioglobina a la circulación e injuria renal aguda de causa tubular-obstructiva e hemodinámica."
  },
  {
    "numb": 24,
    "question": "Con respecto al consumo de alcohol, señale la opción correcta:  ",
    "answer": [
      "a) Los consumidores crónicos de alcohol que consumen cocaína presentan mayor riesgo de hepatotoxicidad. "
    ],
    "options": [
      "a) Los consumidores crónicos de alcohol que consumen cocaína presentan mayor riesgo de hepatotoxicidad. ",
      "b) La hepatopatía alcohólica es menos frecuente cuando se asocia consumo de cocaína.  ",
      "c) El sindrome simpaticomimetico es característico de la intoxicación etílica aguda.  ",
      "d) La injuria renal aguda es debida a la precipitación de metabolitos del etanol a nivel tubular, con la consiguiente lisis celular. "
    ],
    "discipline": "Patologia",
    "comment": "El consumo concomitante de etanol y cocaína conduce a la síntesis transesterificada hepática de cocaetileno, un metabolito tóxico activo. Además, el alcoholismo crónico induce la isoenzima microsamal CYP2E1, incrementando la bioactivación de la cocaína hacia metabolitos hepatotóxicos reactivos con generación severa de estrés oxidativo y necrosis hepática."
  },
  {
    "numb": 25,
    "question": "Con respecto a la nefrotoxicidad por cocaína, marque la opción correcta:  ",
    "answer": [
      "b) El consumo de y daño celular. alcohol asociado incrementa el riesgo de rabdomiólisis y por consiguiente de injuria renal aguda.  "
    ],
    "options": [
      "a) La injuria renal aguda se explica en parte por la precipitación del alcaloide cocaína sobre los túbulos renales, con la consiguiente obstrucción  ",
      "b) El consumo de y daño celular. alcohol asociado incrementa el riesgo de rabdomiólisis y por consiguiente de injuria renal aguda.  ",
      "c) La nefrotoxicidad se manifiesta por una injuria renal irreversible ",
      "d) Cuando hay elementos de nefrotoxicidad no es útil la detección de metabolitos de cocaína en orina. "
    ],
    "discipline": "Patologia",
    "comment": "La cocaína desencadena vasoconstricción renal intensa, hipertermia y colapso muscular (rabdomiólisis). El etanol coingerido agrava de forma sinérgica la rabdomiólisis e hipotensión; la mioglobina libre liberada por el daño muscular genera obstrucción tubular renal, toxicidad citotóxica directa sobre el epitelio tubular e injuria renal aguda."
  },
  {
    "numb": 26,
    "question": "Paciente de 36 años, ADIV, VIH positivo, sin adherencia al TARV. Consulta por cefaleas, fiebre y vómitos de 10 días de evolución. Examen físico: desorientado, fiebre de 39°C, lesiones cutáneas tipo molusco contagioso. Fotofobia, rigidez de nuca y Kernig positivo. Resto del examen s/p. RNM: lesiones periventriculares. Punción lumbar :  líquido claro. ¿Cuál cree que debería ser el primer diagnóstico etiológico a descartar? ",
    "answer": [
      "b) Criptococosis diseminada.  "
    ],
    "options": [
      "a) Histoplasmosis pulmonar.  ",
      "b) Criptococosis diseminada.  ",
      "c) Candidiasis cutánea.  ",
      "d) Aspergilosis cutánea. "
    ],
    "discipline": "Patologia",
    "comment": "En un paciente con inmunodepresión avanzada por VIH no tratado que presenta un cuadro de meningoencefalitis subaguda (fiebre, cefalea, rigidez nucal, LCR claro) acompañado de pápulas cutáneas umbilicadas características que simulan molusco contagioso, el diagnóstico prioritario e ineludible es una infección micótica diseminada por Cryptococcus neoformans."
  },
  {
    "numb": 27,
    "question": "¿Qué factores se consideran vinculados a la progresión de la ERC en el tiempo? Señale la combinación más adecuada. ",
    "answer": [
      "c) Hipertensión arterial, obesidad, acidosis metabólica, uropatía obstructiva "
    ],
    "options": [
      "a) Edad, género masculino, hipertensión arterial, uso de lECA/ARA-2  ",
      "b) Edad, género femenino, hipertensión arterial, diabetes mellitus, anemia ",
      "c) Hipertensión arterial, obesidad, acidosis metabólica, uropatía obstructiva ",
      "d) Hiperuricemia, dislipemia, uso de calcio-antagonistas, diabetes mellitus "
    ],
    "discipline": "Patologia",
    "comment": "Los factores que aceleran la progresión del daño renal crónico incluyen la hipertensión intraglomerular y sistémica, la hiperfiltración asociada a la obesidad, la acidemia que estimula vías profibróticas/endoteliales tubulares y la hipertensión en la vía urinaria provocada por la uropatía obstructiva. La corrección de estos factores detiene la pérdida progresiva de nefronas."
  },
  {
    "numb": 28,
    "question": "Respecto al hepatocarcinoma en pacientes con cirrosis, indique lo correcto: ",
    "answer": [
      "b) Se recomienda realizar screening mediante ecografia de abdomen y alfa-fetoproteina cada 6 meses "
    ],
    "options": [
      "a) Es una complicación poco frecuente ",
      "b) Se recomienda realizar screening mediante ecografia de abdomen y alfa-fetoproteina cada 6 meses ",
      "c) Se recomienda realizar screening mediante ecografia de abdomen y alfa-fetoproteina de forma anual ",
      "d) No se recomienda el screening universal "
    ],
    "discipline": "Patologia",
    "comment": "El hepatocarcinoma (CHC) es la principal neoplasia primaria del hígado y una causa mayor de morbimortalidad en pacientes cirróticos. Las guías clínicas internacionales recomiendan un programa de tamizaje y vigilancia periódica mediante ecografía abdominal cada 6 meses (asociado o no a alfa-fetoproteína), lo que permite detectar nódulos neoplásicos en estadios tempranos candidatos a terapias curativas."
  },
  {
    "numb": 29,
    "question": "Mujer de 20 años asintomática y sin antecedentes personales. Enviada por diagnóstico confiable de enfermedad celíaca en su hermana menor. Su primer gesto diagnóstico es: ",
    "answer": [
      "d) Solicitar anti transglutaminasa con dosificación de IgA "
    ],
    "options": [
      "a) Solicitar una videogastroscopía con biopsias duodenales ",
      "b) Pedir estudio genético ",
      "c) Solicitar péptido deaminado de gliadina y antiendomisio ",
      "d) Solicitar anti transglutaminasa con dosificación de IgA "
    ],
    "discipline": "Patologia",
    "comment": "Los familiares de primer grado de pacientes celíacos tienen un riesgo incrementado de padecer la enfermedad. En sujetos asintomáticos mayores de 2 años, el cribado inicial serológico recomendado es la determinación del anticuerpo anti-transglutaminasa tisular IgA (tTG-IgA) emparejado con la cuantificación de IgA sérica total para descartar una deficiencia selectiva de IgA que generaría falsos negativos."
  },
 
{
    "numb": 30,
    "question": "En la esteatohepatitis alcohólica se halla una combinación de cambios morfológicos característica, aunque no patognomónica, que permite su diagnóstico. Esta es: ",
    "answer": [
      "a) Tumefacción y necrosis de los hepatocitos con cuerpos de Mallory-Denk e infiltrado inflamatorio a neutrófilos. "
    ],
    "options": [
      "a) Tumefacción y necrosis de los hepatocitos con cuerpos de Mallory-Denk e infiltrado inflamatorio a neutrófilos. ",
      "b) Fibrosis a predominio portal con infiltrado inflamatorio linfomononuclear, con cuerpos hialinos en los hepatocitos. ",
      "c) Cambio graso de los hepatocitos con acúmulo de cobre y fibrosis periportal. ",
      "d) Fibrosis en alambre de gallinero perivenacentrolobulillar, hepatocitos en vidrio esmerilado e infiltrado inflamatorio con numerosos plasmocitos. "
    ],
    "discipline": "AnatoP",
    "comment": "La esteatohepatitis alcohólica se caracteriza por la tríada histopatológica clásica: 1) tumefacción (balonización) y necrosis hepatocitaria, 2) presencia de cuerpos de Mallory-Denk (agregados citoplasmáticos eosinófilos de filamentos intermedios de queratina) y 3) infiltrado inflamatorio intralobulillar predominantemente neutrofílico rodeando a los hepatocitos dañados, con frecuencia asociado a esteatosis macrovesicular y fibrosis pericelular/perisinusoidal."
  },
  {
    "numb": 31,
    "question": "Paciente de 22 años, sexo masculino, que consulta por disnea de esfuerzo y coloración amarillenta de piel y mucosas, así como astenia persistente. Se le realiza extensa paraclínicа, destacandose a nivel de la TC la presencia de enfisema y hepatomegalia moderada (grado 2) con nodulaciones parenquimatosas. Del interrogatorio se extrae que la madre y el tío materno presentaron similares cuadros clínicos. En caso de realizarse biopsia hepática, cuáles serían los hallazgos más probables: ",
    "answer": [
      "b) Hepatocitos con inclusiones globulares citoplasmáticas redondeadas PAS positivas diastasa resistentes. "
    ],
    "options": [
      "a) Hepatocitos balonizados (tumefactos) con pigmento biliar intracitoplasmático, necrosis e importante infiltrado inflamatorio linfomononuclear con formación de folículos linfoides. ",
      "b) Hepatocitos con inclusiones globulares citoplasmáticas redondeadas PAS positivas diastasa resistentes. ",
      "c) Esteatosis extensa con depósitos de cobre demostrables con rodamina y orceína. ",
      "d) Depósitos a nivel del hepatocito tingibles con azul de Prusia y Perls, asociado a daño ductular. "
    ],
    "discipline": "AnatoP",
    "comment": "La combinación de enfisema pulmonar panacinar de inicio precoz asociado a daño hepático crónico y agregación familiar sugiere fuertemente un déficit de alfa-1 antitripsina. Debido a la mutación genética, la proteína mal plegada se acumula en el retículo endoplásmico de los hepatocitos, manifestándose histológicamente como glóbulos o inclusiones citoplasmáticas redondas que tiñen intensamente con el reactivo de PAS y resisten la digestión previa con diastasa (PAS-D+)."
  },
  {
    "numb": 32,
    "question": "El nódulo reumatoideo se manifiesta histológicamente como:",
    "answer": [
      "a) un granuloma con necrosis fibrinoide central, rodeada de histiocitos y corona de linfocitos"
    ],
    "options": [
      "a) un granuloma con necrosis fibrinoide central, rodeada de histiocitos y corona de linfocitos",
      "b) área nodular fibrosa con depósitos de inmunocomplejos con fibroplastos jóvenes",
      "c) vasculitis trombosada con necrosis fibrinoide, rodeada de reacción gigantoceluilar",
      "d) acúmulos linfoides con formación de centros germinales con presencia de células gigantes multinucleadas en la periferia"
    ],
    "discipline": "AnatoP",
    "comment": "El nódulo reumatoide es la manifestación extraarticular más frecuente de la artritis reumatoide. Su patrón histopatológico clásico consiste en un granuloma necrotizante formado por una gran zona central de necrosis fibrinoide (colágeno alterado con depósitos de fibrina), delimitada por una empalizada de histiocitos/macrófagos activados y rodeada externamente por una corona o manguito de linfocitos, células plasmáticas y fibroblastos."
  },
  {
    "numb": 33,
    "question": "Paciente de SM con síntomas de RGE de larga data se realiza una endoscopía de control y le encuentran lengüetas asalmonadas, rojas, aterciopeladas, que ascienden desde la unión gastroesofágica. Se toman biopsias. Los hallazgos más probables en este paciente son: ",
    "answer": [
      "c) Metaplasia intestinal con presencia de células caliciformes. "
    ],
    "options": [
      "a) Mucosa de la unión esofagogástrica con epitelio escamoso con acantosis y numerosas pseudohifas de Candida spp. ",
      "b) Metaplasia pseudopilórica con núcleos que muestran pseudoestratificación y atipia. ",
      "c) Metaplasia intestinal con presencia de células caliciformes. ",
      "d) Venas dilatadas y tortuosas que alcanzan los vasos subepiteliales. "
    ],
    "discipline": "AnatoP",
    "comment": "El hallazgo endoscópico de lengüetas mucosas asalmonadas en el esófago distal es característico del esófago de Barrett, una complicación del reflujo gastroesofágico (RGE) crónico. Histológicamente, para confirmar el diagnóstico se requiere la sustitución del epitelio escamoso esofágico por metaplasia cilíndrica de tipo intestinal, cuya seña de identidad patognomónica es la presencia de células caliciformes (goblet cells)."
  },
  {
    "numb": 34,
    "question": "En una paciente de SF, 36 años, con historia de anemia y frecuente distensión abdominal, con anticuerpos antiendomisio y antitrransglutaminasa positivos. Los hallazgos en la biopsia duodenal más probables son: ",
    "answer": [
      "a) Aumento de la linfocitosis intraepitelial, con atrofia variable de las vellosidades y pérdida del borde en cepillo. "
    ],
    "options": [
      "a) Aumento de la linfocitosis intraepitelial, con atrofia variable de las vellosidades y pérdida del borde en cepillo. ",
      "b) Inflamación a linfocitos, plasmocitos y macrófagos con formación de granulomas sin necrosis. ",
      "c) Aplanamiento mucoso con ulceración, presencia de pseudomembranas e infiltrado inflamatorio a predominio de polimorfonucleares neutrófilos. ",
      "d) Expansión de las placas de Peyer con formación de elevaciones en forma de meseta que alcanzan los 8 cm, con numerosos polimorfonucleares neutrófilos en la lámina propia. "
    ],
    "discipline": "AnatoP",
    "comment": "El cuadro clínico-serológico es patognomónico de enfermedad celíaca. Los cambios histopatológicos clásicos en las biopsias del intestino delgado (clasificación de Marsh) corresponden a: 1) incremento de los linfocitos intraepiteliales (mínimo >25 por cada 100 enterocitos), 2) hiperplasia de las criptas y 3) atrofia vellositaria de severidad variable con aplanamiento del microborde apical (pérdida del borde en cepillo)."
  },
  {
    "numb": 35,
    "question": "En relación a los inmunocomplejos presentes en la nefritis lúpica:  ",
    "answer": [
      "c) Son detectables por inmunofluorescencia en todos los estadíos  "
    ],
    "options": [
      "a) Sus depósitos se estudian predominantemente a nivel del intersticio ",
      "b) Los inmunocomplejos y su composición no presentan valor diagnóstico  ",
      "c) Son detectables por inmunofluorescencia en todos los estadíos  ",
      "d) Los característicos están compuestos por C3, IgD e IgG "
    ],
    "discipline": "AnatoP",
    "comment": "La nefritis lúpica es el prototipo de glomerulonefritis mediada por inmunocomplejos. En la inmunofluorescencia directa (IFD), la demostración de depósitos de inmunoglobulinas (IgG, IgA, IgM) y fracciones del complemento (C3, C1q), fenómeno conocido como patrón 'full house', es constante y detectable en todas las clases/estadíos de la clasificación ISN/RPS (de la Clase I a la VI)."
  },
  {
    "numb": 36,
    "question": "Paciente de 28 años, SF, se presenta con diarrea crónica, endoscopía que muestra mucosa en empedrado y úlceras aftoides que afecta el íleon, ciego y colon sigmoides, con mucosa sana entre ambos focos, el cuadro histológico más probable es: ",
    "answer": [
      "d) Compromiso de la mucosa y submucosa por un infiltrado inflamatorio linfomononuclear con granulomas, distorsión de la arquitectura de la mucosa y úlceras aftosas. "
    ],
    "options": [
      "a) Úlceras extensas de base ancha, con mucosa que presenta exudado linfomononuclear asociado a polimorfonucleares que permean las criptas, que sólo afecta la mucosa. ",
      "b) Epitelio mucoso con aumento de la permeación de linfocitos, sin ulceración, granulomas u otras alteraciones. ",
      "c) Mucosa con depósito subepitelial de colágeno que se resalta por Tricrómico, asociado a aumento de los linfocitos intraepiteliales. ",
      "d) Compromiso de la mucosa y submucosa por un infiltrado inflamatorio linfomononuclear con granulomas, distorsión de la arquitectura de la mucosa y úlceras aftosas. "
    ],
    "discipline": "AnatoP",
    "comment": "La presencia de salteo de áreas lesionadas, compromiso ileocecal y sigmoideo, patrón en empedrado y úlceras aftoides orientan al diagnóstico de Enfermedad de Crohn. A diferencia de la colitis ulcerosa, la enfermedad de Crohn presenta una inflamación transmural (que compromete mucosa y submucosa o capas más profundas), infiltrado linfoplasmocitario, agregados linfoideos, distorsión de criptas y la presencia característica de granulomas no caseificantes."
  },
  {
    "numb": 37,
    "question": "En la afectación articular del LES, es característico observar: ",
    "answer": [
      "a) Sinovitis no erosiva. "
    ],
    "options": [
      "a) Sinovitis no erosiva. ",
      "b) Hipertrofia e hiperplasia de partes blandas sinoviales con fibrosis. ",
      "c) Hiperplasia de células sinoviales con infiltrado inflamatorio denso, con formación de pannus, que erosiona el tejido cartilaginoso. ",
      "d) Proliferación de condrocitos articulares formando conglomerados, con fibras de colágeno escindidas. "
    ],
    "discipline": "AnatoP",
    "comment": "La afectación articular en el Lupus Eritematoso Sistémico se manifiesta habitualmente como una artritis inflamatoria o artropatía de Jaccoud no erosiva ni destructiva. A diferencia de la artritis reumatoide, no produce proliferación de pannus agresivo ni erosión marginal de hueso o cartílago articular en los estudios radiológicos."
  },
  {
    "numb": 38,
    "question": "Paciente de 70 años, SF, que se presenta con esplenomegalia, poliglobulia de 6.000.000/dl y Hb de 20g/dl, con JAK2 mutado y BCR/ABL no mutado, que en la médula ósea muestra panmielosis con hiperplasia de las series eritroide, granular y megacariocítica y maduración presente, el planteo más probable es: ",
    "answer": [
      "a) Policitemia Vera "
    ],
    "options": [
      "a) Policitemia Vera ",
      "b) Leucemia mielolde crónica. ",
      "c) Mielofibrosis primaria. ",
      "d) Trombocitemia Esencial. "
    ],
    "discipline": "AnatoP",
    "comment": "La Policitemia Vera es una neoplasia mieloproliferativa crónica caracterizada por el aumento incontrolado de la masa eritrocitaria (poliglobulia y Hb marcadamente elevada), esplenomegalia y la presencia de la mutación somática JAK2 V617F en >95% de los casos (con BCR/ABL negativo). La biopsia de médula ósea expone una marcada panmielosis con hiperplasia trilineal (eritroide, granulocítica y megacariocítica) preservando la maduración de las series."
  },
  {
    "numb": 39,
    "question": "En un paciente de 75 años, con uropatía obstructiva baja crónica, PSA normal, sin dolor o fiebre y hemograma normal, el cuadro histológico más probable es: ",
    "answer": [
      "a) Hiperplasia de patrón nodular a predominio de la zona periuretral, con aumento de las glándulas, que se agrupan y en ocasiones muestran dilatación, sin atipías. "
    ],
    "options": [
      "a) Hiperplasia de patrón nodular a predominio de la zona periuretral, con aumento de las glándulas, que se agrupan y en ocasiones muestran dilatación, sin atipías. ",
      "b) Parénquima prostático con exudado a polimorfonucleares neutrófilos, que permean las glándulas, formando abscesos. у ",
      "c) Proliferación epitelial atípica compuesta por túbulos con ausencia de capa de células basales, infiltrativa. ",
      "d) Presencia de lesión intraepitelial con células basales presentes, compuesta por células de núcleos atípicos con nucleolo evidente. "
    ],
    "discipline": "AnatoP",
    "comment": "La Hiperplasia Benigna de Próstata (HBP) es la causa principal de uropatía obstructiva baja en hombres de edad avanzada. Histológicamente cursa con proliferación nodular no neoplásica de componentes epiteliales y estromales centrados en la zona periuretral y de transición, caracterizada por proliferación glandular bien diferenciada con doble capa celular intacta, ausencia de atipia citológica y eventual formación de ectasias o dilataciones quísticas."
  },
  {
    "numb": 40,
    "question": "El linfoma no Hodgkin (LNH) más frecuente en adultos, se caracteriza por: ",
    "answer": [
      "d) Una proliferación difusa de células de gran talla B (CD20 y CD19 positiva) de rápido crecimiento. "
    ],
    "options": [
      "a) Crecimiento en nódulos rodeados de fibrosis densa con células de gran talla de núcleos polilobulados, acompañados de un ambiente inmunorreactivo de linfocitos, plasmocitos y eosinófilos. ",
      "b) Crecimiento en nódulos compuestos por células que semejan centrocitos y centroblastos en cantidad variable, que suele crecer lentamente. ",
      "c) Ser una proliferación con frecuente afectación de la médula ósea y linfocitosis en sangre periférica, con ganglios pequeños y lenta evolución, que muestra una proliferación a predominio de linfocitos pequeños con centros de proliferación. ",
      "d) Una proliferación difusa de células de gran talla B (CD20 y CD19 positiva) de rápido crecimiento. "
    ],
    "discipline": "AnatoP",
    "comment": "El Linfoma Difuso de Células Grandes B (LDCGB) representa la variante más común de Linfoma No Hodgkin en la población adulta (30-40% de los casos). Se caracteriza por una proliferación difusa y agresiva de inmunoblastos o centroblastos B de gran tamaño que destruyen la arquitectura ganglionar u ocluyen tejidos extraganglionares, expresando intensamente antígenos pan-B como CD19, CD20 y CD79a."
  }
 
 
];
